...
Harness 是 Eva 2.1.0 的异常恢复与正式状态校验协议，不是用户功能入口，也不是所有入口默认加载的全局调度器。
...
2.1.0 不因任务复杂就默认生成任务初始化卡。只有用户要求查看结构化任务状态，或异常恢复确实需要重建任务上下文时才生成。
...
本节保留高价值判断的按需反向审查格式，但 2.1.0 没有独立 Doubt 入口，也不自动触发。只有入口已经按本文件的异常/状态条件加载 Harness，且用户明确要求反向审查时才使用。
