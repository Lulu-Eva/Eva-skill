...
仅在用户询问当前任务适配，或既有创作回捞需要判断时，把“能支撑哪种表达、不能支持哪种结论”融入已有用途或匹配原因；复用证据、置信度和缺失信息，不增加成熟度标签或默认卡片栏目。同一经历可以支撑个人分享，却未必能证明普遍方法。不完整点子仍可按原流程保存，保存或回捞不等于可直接成稿、标题已验证或通过发布审核；回捞不自动更新旧卡、重新评分或启动全库盘点。
