...
| 当前创作以第三方样本辅助结构判断，自己的问题、判断和真实素材已成立 | 内部读取 shared Benchmark，核对机制与原创边界后返回 Create；独立拆解或学习样本交 `../eva-teaching/SKILL.md` |
...
- 对标辅助不能变成照搬；Think/Create 内部调用 Benchmark 后返回原调用者，不自动启动教学。
- “批改我的稿、讲讲问题、教我怎么改”交 Teaching；直接改写或只查开头、节拍、AI 味仍走原入口。Teaching 已获完整修订授权时，按 shared 07 完成全部必要修改；一项讲解不限制修订范围，涉及发布审核则修订后回 Preflight 复核。
