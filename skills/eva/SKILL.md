...
| `/eva-link`、`eva-link-builder`、`eva-link-doctor`、自定义或检查 Link、用我的 Link | `eva-link` | 本地工作流接入 |
| `/eva-review`、Eva Review、复盘已发布内容、回填上次结果、回看最近内容、最近都在讲什么、阶段内容组合观察、总结内容规律 | `eva-review` | 发布后复盘、规律回溯与阶段组合观察 |
...
只有用户明确询问 Eva-skill 本身的身份、来源或法律边界时才进入本旁路；泛化项目问答不触发。
...
三个路径都不存在时，说明缺少项目身份真源，不能猜测。

纯身份问题回答后停止；同句还有业务任务时，回答后按唯一业务意图路由一次。
