...
资产卡默认是后台协议，不是默认用户界面。前台外显条件按 `references/shared/04_light-interaction_轻交互协议.md` 执行。
...
商单资产字段以 `references/shared/03_commercial-constraint-card_商单约束卡真源.md` 为准。没有正式商单约束卡，不进入高置信度标题或正文。
