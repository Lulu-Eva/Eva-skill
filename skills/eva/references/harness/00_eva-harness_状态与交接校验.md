...
Harness 默认在后台运行。前台是否展示 Harness 状态，按 `references/shared/04_light-interaction_轻交互协议.md` 判断。
...
触发条件只读取 `references/shared/04_light-interaction_轻交互协议.md` 的显性 Harness 触发条件。本文件只说明 Harness 如何生成和校验初始化卡，不维护第二套外显条件清单。
...
前台是否可以外显 Harness 字段，只读取 `references/shared/04_light-interaction_轻交互协议.md`。未命中时，不输出后台状态字段、任务初始化卡或失败记录。
...
Harness 不做第一入口路由；第一入口以 `references/entry/00_eva-entry_主入口路由.md` 为准。自动交接只处理“已经有资产之后，能否把资产交给下游”。
...
如果用户动词改变了任务方向，停止自动交接，回 Entry 重新判断入口。例如用户先说“保存这个点子”，随后说“直接写正文”，不能继续按 Memory 处理。
...
| 低置信度判断 | 判断未被证成 | 标注低置信度，输出草案或启动 Doubt |
...
如果用户要快速草稿，但命中高价值条件，先输出低置信度草案；保存、发团队或用于决策前再启动 Doubt。
