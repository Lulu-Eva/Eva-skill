...
```text
python3 scripts/eva_link_check.py --link local-modules/{link-id}/ --strict
```
...
python3 scripts/eva_link_check.py --link local-modules/{link-id}/ --strict --registry .eva/links.json
```
...
```text
python3 scripts/eva_asset_validate.py --asset local-modules/{link-id}/tests/expected-asset.example.json --downstream eva-memory
```
