...
Eva-skill 帮助创作者把模糊想法梳理成更清楚、更有依据的判断，并把它们做成短视频或非虚构自媒体文章。2.4.1 在 2.4.0 的账号阶段性定位与写后人群对位基础上，为短视频加入“节拍与心智推进”协议：生成时减少原地重复、逻辑跳跃和信息拥挤，保留真正有用的场景、证据、解释与停顿；这些判断默认在后台静默完成，不增加普通用户的操作步骤。它同时提供跨学科发散、发布前审核、学习、商单拆解、人设素材与产品服务事实采集、记忆沉淀与盘点、Eva 数据备份、本地工作流接入、发布后复盘和多元视角补充能力，并能根据当前结果推荐下一步。
...
| 建立或复盘账号的阶段定位 | “帮我做一次账号阶段性定位” | 证据允许的候选、工作或阶段定位；条件齐全时才交付主页三件套 |
...
账号阶段性定位回答“这个账号当前先围绕哪个经营假设运行”，并要区分内部事实、用户亲自搜索后回传的平台证据和真实发布反馈。人设素材采集只从真实经历、选择代价和长期判断中挖内容素材与表达资格，不替用户做账号或赛道定位。

### 定位时 Eva 会自动搜索小红书吗？

不会。只要结论依赖当前平台需求、对标、主页惯例、商业痕迹或平台规则，Eva 只负责给出定制搜索任务和观察标准，必须由用户亲自搜索、筛选并贴回材料。证据没有回来时，Eva 最高只能给候选定位，不会越级交付完整主页三件套。
