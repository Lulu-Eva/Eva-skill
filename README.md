...
Eva Skill 帮助创作者把模糊想法梳理成更清楚、更有依据的判断，并把它们做成短视频或非虚构自媒体文章。它同时提供话题人群识别、跨学科发散、发布前审核、学习、商单拆解、本地工作流接入、发布后复盘和多元视角补充能力。

安装时必须同时安装 `eva`、十个用户功能入口和 `eva-shared`，共十二个 Skill 目录。`eva-shared` 不是用户入口，只提供共享协议、schema、脚本和示例。
...
Eva-skill 由璐璐 Eva 持续维护。项目会从真实的内容创作场景和用户需求中持续迭代，在尽量保持既有能力、稳定性与用户体验的前提下，逐步补充更实用的创作工具。

感谢凯瑟琳学姐和梦野学姐为 Eva-skill 生态提供的真实场景与产品灵感：凯瑟琳学姐在使用 Eva-skill 创作内容时提出了“学科发散”的需求，推动 Eva Lens 增加观点形成前的跨学科发散模式；梦野学姐提出了“发布前审核”的需求，推动 Eva Preflight 的设计。这两项能力都来自真实创作过程中发现的问题，也是 Eva-skill 持续生长的一部分。
